...
class CWRUDataset(Dataset):
    def __init__(self, root, mode, resize=84, simclr=False):
        self.simclr = simclr
        self.path = os.path.join(root, 'xichu_细粒度分类')  # image path
    ...
    def __getitem__(self, index):
        ...
        if self.simclr:
            ...
            return torch.tensor(aa,dtype=torch.float).detach(), torch.tensor(bb,dtype=torch.float).detach(),label_
